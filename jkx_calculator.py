...
def write_new_values(curr, usage, total):
    # curr: [curr_hot, curr_cold, curr_elec]
    # usage: [hot_u, cold_u, elec_u]
    with open(VALUES_FILE, 'w', encoding='utf-8') as f:
...
def read_history():
    ...
    with open(HISTORY_FILE, 'r', encoding='utf-8') as f:
        ...
        for row in reader:
            ...
            # поддерживаем старые и новые ключи
            h = row.get('hot_usage')  or row.get('hot')   or '0'
            c = row.get('cold_usage') or row.get('cold')  or '0'
            e = row.get('elec_usage') or row.get('elec')  or '0'
            t = row.get('total_cost') or row.get('total') or '0'
            hot_u.append(float(h))
            cold_u.append(float(c))
            elec_u.append(float(e))
...
class UtilityApp(ctk.CTk):
    ...
    def on_calculate(self):
        ...
        # Если прирост по всем счетчикам нулевой — не добавляем запись
        if hu == 0 and cu == 0 and eu == 0:
            messagebox.showinfo('Нет изменений', 'Показания не изменились — запись в историю не добавлена.')
        else:
            # Вывод результата
            text = (
                f"Итоговая стоимость: {total} ₽\n"
            ...
            # Блокировка полей и включение кнопок «Копировать» и «Новая калькуляция»
            for e in self.entries:
                e.configure(state="disabled")
            self.btn_calc.configure(state="disabled")
            self.btn_copy.configure(state="normal")
            ...
            # Запись в историю и обновление предыдущих значений
            write_new_values(curr, [hu, cu, eu], total)
            self.prev_values = curr
            self.tabview.set("История")
            self._draw_history()
    ...
    def _copy_result(self):
        text = self.result_lbl.cget("text")
    ...
    def _draw_history(self):
        ...
        header = ctk.CTkLabel(
            self.hist_frame,
            text="Дата | Горячая | Холодная | Электр. | Итого",
        ...
        if view == "Общий":
            if totals:
                ax.plot(labels, totals, marker='o', linewidth=2, label='Итого')
